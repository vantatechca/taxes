...
export default function ResearchAssistant({ onClose }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 w-[400px] h-[540px] bg-gray-900 border border-slate-700/50 rounded-2xl shadow-2xl shadow-black/50 flex flex-col z-50 overflow-hidden">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-4 py-3 bg-yellow-900/20 border-b border-yellow-800/30">
          <p className="text-xs text-yellow-400 flex items-center gap-1.5">
            <span>{"⚙️"}</span>
            Connect your Claude API key in Settings to enable AI research assistant
          </p>
        </div>
        ...
        <div className="px-4 py-3 space-y-3 min-h-[60px]">
          {messages.length === 0 ? (
            <p className="text-xs text-slate-500 text-center py-4">
              No conversation yet. AI chat will appear here once connected.
            </p>
          ) : (
            messages.map((msg, i) => {
                  ...
                  <div
                    className={`max-w-[85%] px-3 py-2 rounded-xl text-sm ${
                      isUser
                        ...
                        : "bg-slate-800 text-gray-300 rounded-bl-sm"
                    }`}
                  >
        ...
        <div className="px-4 pb-3">
          <div className="flex items-center gap-2">
            <input
              ...
              placeholder="AI assistant not connected..."
              disabled
              className="flex-1 bg-slate-800/80 border border-slate-600/50 rounded-lg px-3 py-2 text-sm text-gray-100 placeholder-slate-600 focus:border-indigo-500 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
            />
            <button
              disabled
              className="bg-indigo-600 disabled:opacity-50 disabled:cursor-not-allowed text-white px-3 py-2 rounded-lg transition-colors shrink-0"
            >
          ...
          <div className="bg-slate-800/80 border border-slate-600/50 rounded-xl p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white px-3 py-1 rounded-lg text-xs font-medium flex items-center gap-1 transition-colors"
              >
                {rulesSaving && <Spinner />}
                Save Rules
              </button>
            ...
            {rulesLoading ? (
              ...
              <div className="space-y-2">
                ...
                {rules.map((rule, idx) => (
                  ...
                  >
                    <span className="text-xs text-slate-500 mt-1 w-5 shrink-0">
                      {idx + 1}.
                    </span>
                    {editingRuleIdx === idx ? (
                      <div className="flex-1 flex items-center gap-1">
                        <input
                          ...
                          className="flex-1 bg-slate-700 border border-slate-600 rounded px-2 py-1 text-xs text-gray-100 focus:outline-none focus:border-indigo-500"
                          autoFocus
                        />
                        <button
                      ...
                      <>
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                          <button
                            ...
                            className="text-xs text-slate-500 hover:text-indigo-400 transition-colors px-1"
                          >
                            Edit
                          </button>
                          <button
                ...
                <div className="flex items-center gap-2 mt-2 pt-2 border-t border-slate-700/50">
                  <input
                    ...
                    className="flex-1 bg-slate-700 border border-slate-600 rounded px-2 py-1.5 text-xs text-gray-100 placeholder-slate-500 focus:outline-none focus:border-indigo-500"
                  />
                  <button
